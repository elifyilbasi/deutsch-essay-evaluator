...
const B2: LevelRubric = {
  ...
  criteria: [
    ...
    },
    {
      ...
      label: "Kriterium II: Kommunikative Gestaltung",
      description:
        "Judge the range of expression together with structure and text logic — Kohäsion and Kohärenz: Textlogik, Konnektoren, Register and Wortschatzspektrum, and the discourse-steering elements that bind the utterances into one semantic whole. THE TEXTSORTE IS A SEMI-FORMAL OR FORMAL E-MAIL, so the letter's features — Absender, Empfänger, Datum — are explicitly nicht gefordert and their absence must never cost marks, whatever the task sheet's own advice says. Withhold A only when the Textsortenmerkmale of a semi-formal or formal message (Betreffzeile, Anrede, Schlussformel) are missing AND the Wortschatzspektrum is not fully adequate — both together, never one alone. Withhold B if the wrong register is chosen or the register wavers, if the Wortschatzspektrum is not adequate for B2, or if the Leitpunkte are listed linearly without logical connection. A word that is well formed but wrong in context counts under that second ground: where the sentence ends up stating something absurd, or the recipient has to work out what was meant, the Wortschatzspektrum IS inadequate for B2 and B is withheld. Fluent German around the error is not a reason to excuse it — a text can read smoothly and still not say what it means. Judge repeated self-contradiction the same way, whether between two statements or against the Situierung: utterances that cancel one another out are not bound into the semantic whole this criterion opens by asking for. Two limits on that, and they matter as much as the rule. Band B tolerates \"Lücken im Wortschatz\" in as many words, so one unfortunate word in a text that is otherwise precise is a gap and not a failure — the line is whether the recipient can still tell what was meant, not whether every choice was the best one. And a wrong case, gender or agreement is grammar, which telc marks under Kriterium III: mark the wrong WORD here and the wrong FORM there, so that one error is not charged twice against criteria telc states are independent. Give C where Textlogik, connectives, vocabulary range and register are predominantly unsuitable and would make a negative impression on the recipient, and D where they are wholly unsuitable.",
